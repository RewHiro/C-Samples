...
int main() {
	...
	std::array<PlayerData, 2> players_data = {
	...
	while (env.isOpen()) {
		...
		for(int i = 0; i < 2; i++){
			Update(env, uies.at(i), players_data.at(i), dis);
		}
		for (int i = 1; i >= 0; i--){
			if (env.isPushButton(Mouse::LEFT)){
				if (PointToQuad(env.mousePosition(), uies.at(i).pos, uies.at(i).size)){
					uies.at(i).is_drag = true;
					dis = env.mousePosition() - uies.at(i).pos;
					break;
				}
			}
		}
		...
		env.setupDraw();
		for (int i = 0; i < 2; i++){
			StatusUI(uies.at(i), players_data.at(i));
		}

		...
		static 	std::unordered_map<EaseType, ease_func>ease_list = {
			...
			},
			{
				...
				[](float t, const T& b, const T& c, const float d){
					if (t == 0)         return b;
					if (t == d)         return b + c;
					if ((t /= d / 2) < 1) return static_cast<float>(c / 2 * std::pow(2, 10 * (t - 1)) + b);
					return static_cast<float>(c / 2 * (-std::pow(2, -10 * --t) + 2) + b);
				}
			},
			{
				EaseType::IN_CIRC,
			...
			},
			{
				...
				[](float t, const T& b, const T& c, const float d){
					...
					float s = p / 4;
					t -= 1;
					return static_cast<float>(-(a * std::pow(2, 10 * t) * std::sin((t*d - s)*(2 * M_PI) / p)) + b);
				}
			},
			{
				...
				[](float t, const T& b, const T& c, const float d){
					...
					T a(c);
					float s = p / 4;
					return static_cast<float>((a* std::pow(2, -10 * t) * std::sin((t*d - s)*(2 * M_PI) / p) + c + b));
				}
			},
			{
				...
				[](float t, const T& b, const T& c, const float d){
					...
					{
						t -= 1;
						return static_cast<float>(-.5*(a * std::pow(2, 10 * t) * std::sin((t*d - s)*(2 * M_PI) / p)) + b);
					}
					t -= 1;
					return static_cast<float>(a * std::pow(2, -10 * t) * std::sin((t*d - s)*(2 * M_PI) / p)*0.5f + c + b);
				}
			},
			{
				EaseType::IN_BOUNCE,
			...
			},
			{
				...
				[](float t, const T& b, const T& c, const float d){
					if ((t /= d) < (1 / 2.75f)) {
						...
						t -= (1.5f / 2.75f);
						return static_cast<double>(c*(7.5625f*t*t + .75f) + b);
					}
					else if (t < (2.5f / 2.75f)) {
						t -= (2.25f / 2.75f);
						return static_cast<double>(c*(7.5625f*t*t + .9375f) + b);
					}
					else {
						t -= (2.625f / 2.75f);
						return static_cast<double>(c*(7.5625f*t*t + .984375f) + b);
					}
				}
			},
			{
				...
				[](float t, const T& b, const T& c, const float d){
					if (t < d / 2) return (ease_list.find(EaseType::IN_BOUNCE)->second)(t * 2, T(b * 0), c, d) * .5f + b;
					else         return (ease_list.find(EaseType::OUT_BOUNCE)->second)(t * 2 - d, T(b * 0), c, d) * .5f + c*.5f + b;
				}
			},
		};
